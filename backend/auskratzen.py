...
def stl_ausrichtung_und_vermessung(eingabe_datei_name, ausgabe_datei_name):
...
class Ebene:
    ...
    def __str__(self):
        ausgabe = (
            f"Ebene aus {self.punktzahl} Punkten\n"
...
class Krone:
    ...
    def __init__(self, stl_datei_name, punkte_datei_name, ausgabe=True):
        ...
        """
        self.markierte_punkte = np.loadtxt(punkte_datei_name, dtype=np.float32)
        self.mesh = om.read_trimesh(
            stl_datei_name, binary=True, vertex_normal=True
        )
        if ausgabe:
            logging.debug(
                f"Datei {stl_datei_name} "
    ...
    def status_update(self, ausgabe):
        ...
        self.ebene_basis = Ebene(
            self.markierte_punkte[1:], self.pos_stuetze - basis_schwerpunkt
        )
        self.vektor_basis_stuetze = (
            self.pos_stuetze - self.ebene_basis.schwerpunkt
        )
        ...
        nachbarn = self.nachbarschaft(
            self.v_stuetze, STUETZEN_EBENEN_RADIUS_MM
        )
        punkte_array = np.array(
            [self.mesh.point(n) for n in nachbarn], dtype=np.float32
        )
        self.ebene_occ = Ebene(
            punkte_array, basis_schwerpunkt - self.pos_stuetze
        )
        winkel = math.degrees(
            math.acos(
                np.dot(
                    self.ebene_basis.normalenvektor,
        ...
        if ausgabe:
            logging.debug(
                f"Winkel zwischen der oberen und unteren Normalen:"
                f"{winkel:.1f} Grad"
            )
        #
        ...
        self.occ_radius = (
            np.mean(np.linalg.norm(punkte_array - schwerpunkt, axis=1)) / 2.0
        )
        if ausgabe:
        ...
        self.mittelpunkt_occ = (
            self.pos_stuetze - self.richtung_basis_stuetze * self.occ_radius
        )
        ...
        if (
            np.dot(self.mesh.normal(self.v_stuetze), self.vektor_basis_stuetze)
            > 0
        ):
            if ausgabe:
    ...
    def update_octree(self):
        self.octree = ot.PyOctree(
            self.mesh.points(), self.mesh.face_vertex_indices()
        )
        ...
        while queue:
            ...
            for v in self.mesh.vv(v_handle):
                ...
                if v_idx not in schon_betrachtet:
                    distanz_mm = np.linalg.norm(self.mesh.point(v) - pos_mitte)
    ...
    def verschiebungs_richtung(self, v_handle, stuetzen_durchmesser_mm):
        ...
        if d_occ < self.occ_radius:
            ...
            basis_punkt = (
                self.ebene_occ_zentrum.schwerpunkt
    ...
    def schrumpfen(
        ...
        while queue:
            ...
            if d_occ < 0:
                ...
                ziel_dicke = self.uebergang(
                    d_occ, okklusal_dicke_mm, seiten_dicke_mm, sigma
                )
            else:
                d_basis = self.ebene_basis.distanz(v_pos)
                ziel_dicke = self.uebergang(
                    d_basis - rand_breite_mm, 0, seiten_dicke_mm, sigma
                )
            ...
            for v in self.mesh.vv(v_handle):
                v_idx = v.idx()
                if (
                    v_idx not in schon_betrachtet
                    and self.ebene_basis.distanz(self.mesh.point(v))
                    >= rand_breite_mm / 2
                ):
                    queue.append(v)
                    schon_betrachtet.add(v_idx)
        ...
        self.update_octree()
        #
        # om.write_mesh('/tmp/geschrumpft.stl', self.mesh, binary=True)
    ...
    def auskratzen(
        ...
        while queue:
            ...
            v_idx = v_handle.idx()
            v_pos = self.mesh.point(v_handle)
            d_stuetze = np.linalg.norm(
                self.mesh.point(v_handle) - self.pos_stuetze
            )
            d_basis = self.ebene_basis.distanz(v_pos)
            if d_stuetze > stuetzen_radius_mm and d_basis > rand_breite_mm / 2:
                ...
                strahl_array = np.array(
                    [v_pos + 0.01 * v_richtung, v_pos + 1.01 * v_richtung],
                    dtype=np.float32,
                )
        ...
        for idx in range(len(self.mesh_points)):
            if idx in schon_betrachtet and idx not in octree_fehler_menge:
                zaehler = 0
                ...
                mesh_points[idx] = v_pos + verschiebung * v_richtung * v_anteil
        # Glaettung
        anzahl_ausreisser = 0
        ...
        for idx in octree_fehler_menge:
            ...
            for v in self.mesh.vv(self.mesh.vertex_handle(idx)):
                ...
                zaehler += 1
            if zaehler >= 3:
                mittlere_nachbar_position = np.mean(
                    vektor_array[:zaehler], axis=0
                )
                mittlere_nachbar_std = np.std(
                    np.linalg.norm(
                        vektor_array[:zaehler] - mittlere_nachbar_position,
                        axis=0,
                    ),
                    ddof=1,
                )
...
def modell_auskratzen(
    stl_datei_name_eingabe,
    punkte_datei_name,
    stuetzen_durchmesser_mm,
    rand_breite_mm,
    seiten_dicke_mm,
    ...
    krone_skaliert.schrumpfen(
        seiten_dicke_mm,
        okklusal_dicke_mm,
        stuetzen_durchmesser_mm / 2,
        rand_breite_mm,
        uebergangs_breite_mm,
    )
    #
...
def auskratzen_main():
    ...
    try:
        ...
        print('Konfigurationsdatei "' + conf_file_name + '" nicht gefunden.')
        exit(0)
    ...
    volumen_pre = stl_ausrichtung_und_vermessung(
        stl_eingabe_datei_name, stl_korrigiert_datei_name
    )
    #
    punkte_datei_name = temp_datei_name("txt")
    positionen_markieren(stl_korrigiert_datei_name, punkte_datei_name)
    #
    stuetzen_durchmesser_mm = config.getfloat(
        "DEFAULT", "stuetzen_durchmesser_mm"
    )
    ...
    modell_auskratzen(
        ...
        punkte_datei_name,
        stuetzen_durchmesser_mm,
        kronenrand_breite_mm,
        seiten_dicke_mm,
        okklusal_dicke_mm,
        uebergangs_breite_mm,
        stl_ausgabe_datei_name,
    )
    volumen_post = stl_ausrichtung_und_vermessung(stl_ausgabe_datei_name, None)
